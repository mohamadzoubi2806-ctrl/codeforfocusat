  ...
  const submitReportToTeacher = async (e: React.FormEvent) => {
    ...
    try {
      await fetch(`https://formspree.io/f/${FORM_ID}`, {
        ...
        body: JSON.stringify({
          subject: "New Student Lead (English) + Chat Log",
      ...
      <main className="max-w-4xl mx-auto px-4 py-12 relative">
        <AnimatePresence>
          {reportSent && (
            ...
            >
              <span>✅ Sent successfully! We will contact you soon.</span>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden min-h-[500px] flex flex-col relative">
          <div className="flex-1 p-6 overflow-y-auto space-y-6 max-h-[500px]">
            {messages.map((msg, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex items-start gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`p-4 rounded-2xl max-w-[80%] font-paragraph leading-relaxed ${msg.role === 'assistant' ? 'bg-gray-100 text-foreground rounded-tl-none' : msg.role === 'error' ? 'bg-red-50 text-red-600' : 'bg-primary text-white rounded-tr-none'}`}>
                  {msg.content}
                </div>
              </motion.div>
            ))}
          ...
          {messages.length > 2 && (
            <div className="px-6 py-2 bg-yellow-50 border-t border-yellow-100 flex justify-between items-center">
              <span className="text-xs text-yellow-800 font-paragraph">Need more help?</span>
              <button onClick={handleOpenReport} className="flex items-center gap-2 text-xs font-heading bg-yellow-500 text-white px-3 py-1.5 rounded hover:bg-yellow-600 transition-colors"><Mail size={14} /> Send to Teacher</button>
            </div>
          )}

          <div className="p-4 bg-gray-50 border-t border-gray-200">
            <form onSubmit={handleSend} className="flex gap-3">
              <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Type your question..." className="flex-1 px-5 py-3 rounded-xl border border-gray-300 focus:outline-none focus:border-primary font-paragraph" disabled={loading} />
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/50 z-10 flex items-center justify-center p-4 backdrop-blur-sm">
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl">
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-heading font-bold text-foreground">Contact Teacher</h3>
                    <button onClick={() => setShowContactModal(false)} className="text-gray-400 hover:text-red-500"><X size={20} /></button>
                  </div>
                  <p className="text-sm text-gray-600 mb-4 font-paragraph">Enter your phone or email, and the teacher will review this chat and contact you.</p>
                  <form onSubmit={submitReportToTeacher} className="space-y-4">
                    <input type="text" value={studentContact} onChange={(e) => setStudentContact(e.target.value)} placeholder="Phone number or Email" className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-primary focus:outline-none font-paragraph" required />
                    <button type="submit" disabled={sendingReport} className="w-full bg-primary text-white py-3 rounded-xl font-bold hover:bg-destructive transition-colors disabled:opacity-50">{sendingReport ? 'Sending...' : 'Send Request'}</button>
                  </form>
                </motion.div>
              </motion.div>
            )}
